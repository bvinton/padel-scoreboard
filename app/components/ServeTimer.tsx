...
export default function ServeTimer({ timerStarted, timeLeft, isOutdoorMode }: ServeTimerProps) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-[9999] overflow-hidden">
      {timeLeft > 0 && (
        // Brought inside the screen by 2px so the solid line is fully visible
        <div className="absolute" style={{ inset: '2px' }}>
          <svg className="w-full h-full overflow-visible" style={{ filter: getGlowFilter() }}>
            {/* Increased strokeWidth to 6 for baseline visibility */}
            <line x1={`${getTopLeftX1()}%`} y1="0%" x2="50%" y2="0%" stroke="currentColor" strokeWidth="6" className={`transition-all duration-75 ease-linear ${getTextColorClass()}`} />
            <line x1="50%" y1="0%" x2={`${getTopRightX2()}%`} y2="0%" stroke="currentColor" strokeWidth="6" className={`transition-all duration-75 ease-linear ${getTextColorClass()}`} />
            <line x1="0%" y1="0%" x2="0%" y2={`${getSideY2()}%`} stroke="currentColor" strokeWidth="6" className={`transition-all duration-75 ease-linear ${getTextColorClass()}`} />
            <line x1="100%" y1="0%" x2="100%" y2={`${getSideY2()}%`} stroke="currentColor" strokeWidth="6" className={`transition-all duration-75 ease-linear ${getTextColorClass()}`} />
            <line x1="0%" y1="100%" x2={`${getBottomLeftX2()}%`} y2="100%" stroke="currentColor" strokeWidth="6" className={`transition-all duration-75 ease-linear ${getTextColorClass()}`} />
            <line x1={`${getBottomRightX1()}%`} y1="100%" x2="100%" y2="100%" stroke="currentColor" strokeWidth="6" className={`transition-all duration-75 ease-linear ${getTextColorClass()}`} />
          </svg>
        </div>
      )}
