...
export default function MatchSetupModal({ isOpen, onClose, setRosterOpen, setHistoryOpen, setArchiveOpen }: MatchSetupModalProps) {
  const { language, isOutdoorMode, matchType, toggleMatchType, clearAllPlayers, toggleServer } = useMatchStore();
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[200] p-4" onClick={onClose}>
      <div className={`${bgColor} border ${isOutdoorMode ? 'border-gray-300' : 'border-slate-700'} rounded-2xl w-full max-w-sm flex flex-col shadow-2xl animate-in zoom-in-95 duration-200`} onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          <button onClick={() => { clearAllPlayers(); onClose(); }} className={`w-full py-3 rounded-xl font-black uppercase text-xs tracking-widest transition-all flex justify-center items-center gap-3 border ${isOutdoorMode ? 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100' : 'bg-red-900/10 text-red-400 border-red-900/30 hover:bg-red-900/20'}`}>
            <UserMinus size={18} /> Clear Active Players
          </button>

          {/* NEW: Swap Server Button moved here */}
          ...
          <button onClick={() => { setRosterOpen(true); onClose(); }} className={`w-full py-4 px-5 rounded-xl font-black uppercase tracking-wider transition-all flex items-center gap-4 border ${panelColor}`}>
