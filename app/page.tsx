    ...
    <main className="h-screen w-full flex flex-col bg-black text-white select-none overflow-hidden p-1 font-sans">
      ...
      {matchWinner && !matchWinnerDismissed && !localDismissed && (
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl animate-in fade-in duration-500" onClick={() => setLocalDismissed(true)}>
          <div className="relative flex flex-col items-center bg-slate-900 border-4 md:border-8 border-amber-400 p-8 md:p-16 rounded-3xl md:rounded-[4rem] text-center shadow-[0_0_100px_rgba(251,191,36,0.4)]" onClick={e => e.stopPropagation()}>
            ...
            <span className="absolute -bottom-8 md:-bottom-16 -left-8 md:-left-16 text-5xl md:text-8xl animate-pulse">🎊</span>
            <span className="absolute -bottom-8 md:-bottom-16 -right-8 md:-right-16 text-5xl md:text-8xl animate-pulse delay-300">🎉</span>
            <Trophy className="w-16 h-16 md:w-24 md:h-24 text-amber-400 mb-4 md:mb-8 animate-pulse" />
            <h2 className="text-5xl md:text-8xl font-black mb-2 md:mb-4 text-white italic uppercase tracking-tighter">
              {matchWinner === 'team1' ? team1Name : team2Name}
            </h2>
            <h3 className="text-2xl md:text-4xl font-black text-amber-400 uppercase italic mb-6 md:mb-12 tracking-[0.2em]">Victory Fanfare</h3>
        ...
        <div className="absolute inset-0 z-[60] bg-black/95 flex items-center justify-center p-2 md:p-4" onClick={() => setHistoryOpen(false)}>
          <div className="bg-slate-900 border-2 md:border-4 border-slate-700 p-4 md:p-10 rounded-2xl md:rounded-[3rem] w-full max-w-sm md:max-w-2xl flex flex-col gap-3 md:gap-6 max-h-[90vh]" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto flex flex-col gap-2 md:gap-3 pr-2">
              {historyLog.length === 0 ? (
                <div className="text-center text-slate-600 font-bold py-6 md:py-10 text-lg md:text-2xl uppercase italic">No points scored yet</div>
              ) : (
                historyLog.map(log => (
            ...
            <button onClick={() => setHistoryOpen(false)} className="py-3 md:py-6 bg-white text-black text-xl md:text-3xl font-black rounded-xl md:rounded-3xl uppercase mt-2 md:mt-4 active:scale-95 transition-transform">Close Log</button>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 z-50 bg-black/95 flex items-center justify-center p-2 md:p-4" onClick={() => setSettingsOpen(false)}>
          <div className="bg-slate-900 border-2 md:border-4 border-slate-700 p-4 md:p-8 rounded-2xl md:rounded-[3rem] w-full max-w-md md:max-w-xl flex flex-col gap-2 md:gap-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
             ...
             <div className="grid grid-cols-2 gap-2 md:gap-4">
               <input value={team1Name} onChange={e => setTeam1Name(e.target.value)} placeholder="TEAM 1" className="bg-slate-800 border-2 md:border-4 border-slate-700 rounded-xl md:rounded-3xl p-2 md:p-5 text-white text-sm md:text-2xl font-black uppercase text-center focus:border-indigo-500 outline-none transition-colors" maxLength={15} />
               <input value={team2Name} onChange={e => setTeam2Name(e.target.value)} placeholder="TEAM 2" className="bg-slate-800 border-2 md:border-4 border-slate-700 rounded-xl md:rounded-3xl p-2 md:p-5 text-white text-sm md:text-2xl font-black uppercase text-center focus:border-indigo-500 outline-none transition-colors" maxLength={15} />
             </div>

             <button onClick={toggleFullscreen} className="py-2 md:py-5 rounded-xl md:rounded-3xl bg-indigo-600 border-2 md:border-4 border-white text-sm md:text-2xl font-black uppercase flex items-center justify-center gap-2 md:gap-4 transition-transform active:scale-95">
             ...
             <button onClick={() => setSettingsOpen(false)} className="py-2 md:py-5 bg-white text-black text-sm md:text-2xl font-black rounded-xl md:rounded-3xl uppercase mt-1 md:mt-2">Close Settings</button>
          </div>
        </div>
      )}
      ...
      <section className="h-[92%] flex flex-col gap-1">
        {[ { id: "team1", data: team1, label: team1Name }, { id: "team2", data: team2, label: team2Name } ].map((t) => (
          <button 
            ...
            className={`flex-1 rounded-xl md:rounded-[1.5rem] border-[3px] md:border-[6px] flex flex-row items-center relative transition-all ${
      ...
      <footer className="h-[8%] flex items-center justify-between px-2 md:px-10 border-t border-slate-900 bg-slate-950/50">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <button onClick={() => setSettingsOpen(true)} className="p-2 md:p-3 bg-slate-900/50 border border-slate-800 rounded-lg md:rounded-2xl text-slate-600 hover:text-white transition-all active:scale-95">
